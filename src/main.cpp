...
bool CTxOut::IsDust() const
{
    ...
    // so dust is a txout less than 54 uEUR
                              ...
                              enum GetMinFee_mode mode) const
{
    ...
    if (fAllowFree)
    {
        if (nBlockSize == 1)
        {
            ...
            // (about 4500 EUR if made of 50 EUR inputs)
            if (nBytes < 10000)
                nMinFee = 0;
        }
        else
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
	...
	/*
		Total Coins: 126 Million
		* 1st 5 Months, 63 Million Coins will be generated
		  Every 21,000 Blocks (1 Month) the reward steps down from 1500, 750, 375, 225, 150.
		
		* Through the next few decades the Remaining 63 Million will be generated
		  Every 420,000 Blocks (2 Years), The reward starts at 75 and is Halved each period
		  31.5 Million come from first 2 Years of 420K Blocks
	*/
	...
	else if (nHeight >= BlockCountA * 4) { nSubsidy = 150 * COIN; }
	else if (nHeight >= BlockCountA * 3) { nSubsidy = 225 * COIN; }
	else if (nHeight >= BlockCountA * 2) { nSubsidy = 375 * COIN; }
	else if (nHeight >= BlockCountA) { nSubsidy = 750 * COIN; }
	else if (nHeight < 3) {nSubsidy = 3780000 * COIN;}
	else if (nHeight < 2) {nSubsidy = 1260000 * COIN;}
	else if (nHeight < 1) {nSubsidy = 0 * COIN;}
	else { nSubsidy = 1500 * COIN; }
...
unsigned int static KimotoGravityWell(const CBlockIndex* pindexLast, const CBlockHeader *pblock, uint64 TargetBlocksSpacingSeconds, uint64 PastBlocksMin, uint64 PastBlocksMax) {
	/* current difficulty formula, eurocoin - kimoto gravity well */
...
void ThreadScriptCheck() {
    RenameThread("eurocoin-scriptch");
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0xf6b9cf74c5fd9297019d5452dddf01f97631f2d62adee72fe2284cf433429fe4");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "Eurocoin - Started";
        ...
        block.nTime    = 1396465001;
        block.nNonce   = 386362569;
        ...
        assert(block.hashMerkleRoot == uint256("0x6b2ee585c0a5c555c2527e531dc4c40c2fcc992895880903e218a629369b7280"));
...
void static EurocoinMiner(CWallet *pwallet)
{
    printf("EurocoinMiner started\n");
    ...
    try { loop {
        ...
        printf("Running EurocoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("EurocoinMiner terminated\n");
        throw;
    }
}

void GenerateEurocoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&EurocoinMiner, pwallet));
}

...
bool RecvLineFixed(SOCKET hSocket, string& strLine)
{
    ...
    loop
    {
		...
		// +eurocoin added for irc exits, without this, recv(hSocket, &c, 1, 0); will hang when closing the program after using irc on windows.
		bool stop = false;
		u_long RecvCount;
        while(!stop)
        {
...
void ThreadIRCSeed()
{
    ...
    while (true)
    {
        ...
        if (fTestNet) {
            Send(hSocket, "JOIN #EUROC0INTEST3\r");
            Send(hSocket, "WHO #EUROC0INTEST3\r");
        } else {

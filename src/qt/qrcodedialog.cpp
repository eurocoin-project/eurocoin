...
QString QRCodeDialog::getURI()
{
    ...
    if (ui->chkReqPayment->isChecked())
    {
        ...
        {
            // even if we allow a non EUR unit input in lnReqAmount, we generate the URI with EUR as unit (as defined in BIP21)
            ret += QString("?amount=%1").arg(EurocoinUnits::format(EurocoinUnits::EUR, ui->lnReqAmount->value()));
            paramCount++;
        }
        else
        {
